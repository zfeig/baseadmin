    ...
    <div class="row">
        <div class="col-md-12">
            <div class="main">
                <h2>角色列表</h2>
                <div class="tool">
                    <a class="btn btn-primary btn-sm " href="<?=Yii::$app->urlManager->createUrl('admin/auth/roleadd')?>">添加角色</a>
                </div>
                <table class="table table-hover">
                    ...
                    <?php if(count($roles)>0):?>
                        <?php foreach($roles as $k=> $v):?>
                            <tr>
                                <td><?=$k?></td>
                                <td><a href="<?=Yii::$app->urlManager->createUrl(['admin/auth/rolereview','id'=>$v->id])?>" class="btn btn-group review"><?=$v->role?></a></td>
                                <td><?=$v->desc?></td>
                                <td>

                                    <a class="btn btn-sm btn-success" href="<?=Yii::$app->urlManager->createUrl(['admin/auth/roleupdate','id'=>$v->id])?>">编辑</a>

                                    <a class="btn btn-sm btn-danger msgshow"  href="<?=Yii::$app->urlManager->createUrl(['admin/auth/roledel','id'=>$v->id])?>">删除</a>
